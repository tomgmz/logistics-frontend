...
function getEncodedPolyline(routeData: OptimizeRouteResponse | null): string | null {
  ...
  const ext = routeData as OptimizeRouteResponse & { encoded_polyline?: string; encodedPolyline?: string }
...
function TransitBookingRow({
  ...
  const lastDestAddress = booking.booking_destinations?.[booking.booking_destinations.length - 1]?.address
  ...
  return (
    ...
    >
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center z-[1]" style={{ background: 'rgba(10,10,10,0.65)' }}>
      ...
      <div className="flex items-center gap-1.5 mb-2">
        <span className="text-white text-[10px] font-bold min-w-[24px]">{addressAbbr(booking.origin, 'ORG')}</span>
...
function RouteMarkers({
  ...
  return (
    <>
      <AdvancedMarker position={{ lat: routeData.origin.latitude, lng: routeData.origin.longitude }} title={routeData.origin.address}>
        <div
      ...
      {stops.map((stop) => {
        const bg = stop.status === 'delivered' ? '#3af626' : stop.status === 'failed' ? '#f62626' : '#4df9ed'
        return (
          <AdvancedMarker
...
export default function TransitTrackingView() {
  ...
  useEffect(() => { if (!user) return; loadBookings() }, [loadBookings, user])
  ...
  const filteredList = useMemo(() => filterBookings(searchFiltered, activeFilter), [searchFiltered, activeFilter])
  ...
  const detailPanel = routeData && bookingDetail ? (
    <DetailsPanelContent
      routeData={{
        ...routeData,
        total_duration: resolvedDuration > 0 ? Math.floor(resolvedDuration / 60) : routeData.total_duration,
      }}
    ...
    <div className="flex flex-col items-center justify-center py-16 gap-3">
      <div className="w-10 h-10 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'var(--color-cyan)' }} />
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-2 pb-3 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10">
              {listLoading ? (
                ...
                <div className="p-4 text-center">
                  ...
                  <button type="button" onClick={loadBookings} className="text-[var(--color-cyan)] text-sm inline-flex items-center gap-1">
                    <RefreshIcon sx={{ fontSize: 14 }} /> Retry
                  </button>
                </div>
              ) : filteredList.length === 0 ? (
                <p className="text-center text-white/45 text-sm py-8 px-2">No bookings in this filter. Try &quot;All&quot; or adjust search.</p>
              ) : (
                <AnimatePresence>
