  ...
  return (
    <div className="flex flex-col h-full overflow-auto p-4 lg:p-6 gap-4 lg:gap-6">
      <AnimatePresence mode="wait">
        {submitted ? (
          <SuccessView key="success" onNewBooking={() => dispatch(resetBooking())} />
        ) : (
          <motion.div key="review" variants={stagger} initial="hidden" animate="show"
                  ...
                  {vehicle ? (
                    <>
                      <div className="relative w-full h-[180px] lg:h-[220px]">
                        <Image src={vehicle.imageUrl || '/images/vehicles/default-truck.png'}
                          alt={vehicle.name} fill className="object-contain drop-shadow-2xl" />
                      </div>
            ...
            <motion.div variants={fadeUp}
              ...
              <div className="flex items-center gap-3 mb-4">
                <span className="px-3 py-1 rounded-sm bg-[var(--color-cyan)] text-[var(--color-bg)] font-body booking-text text-xs font-bold uppercase tracking-widest">
                  {mode} Cargo
                </span>
                    ...
                    <div key={section.dropoffIndex} className="flex flex-col gap-3">
                      ...
                      {section.groups.map((g, i) => (
                        <motion.div key={g.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                          transition={{ delay: i * 0.06 }} className="flex flex-col gap-1">
                            ...
                            <div className="flex flex-col divide-y bg-[#424242] divide-white/[0.06]">
                              ...
                              {mode === 'loose' && (
                                <>
                                  ...
                                  <DetailRow label="Dimensions (L × W × H)"
                                    value={g.looseLength && g.looseWidth && g.looseHeight
                              ...
                              {mode === 'palletized' && (
                                <>
                                  ...
                                  <DetailRow label="Dimensions (L × W × H)"
                                    value={g.palletLength && g.palletWidth && g.palletHeight
                                      ? `${g.palletLength} × ${g.palletWidth} × ${g.palletHeight} cm` : '—'} />
                                  <DetailRow label="Gross Weight / Pallet"
                                    value={g.grossWeightPerPallet ? `${g.grossWeightPerPallet} ${g.palletWeightUnit}` : '—'} />
                                  <DetailRow label="Net Weight / Pallet"
                                    value={g.netWeightPerPallet ? `${g.netWeightPerPallet} ${g.palletWeightUnit}` : '—'} />
...
function SectionLabel({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <h3 className={`font-body booking-text text-white font-bold tracking-wide ${className}`}>{children}</h3>
}
...
function InfoBox({ label, value, className = '' }: { label?: string; value: string; className?: string }) {
  return (
    <div className={`rounded-lg border border-white/[0.10] bg-white/[0.03] px-3 py-2 ${className}`}>
      {label && (
        <p className="font-body booking-text text-[var(--color-muted)] text-[10px] lg:text-xs uppercase tracking-widest leading-none mb-1">{label}</p>
      )}
    ...
    <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <motion.button initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }}
        ...
        className="mt-2 px-8 py-3 rounded-xl font-body booking-text font-bold uppercase tracking-widest
                   text-base lg:text-lg text-[var(--color-bg)] bg-[var(--color-cyan)] cursor-pointer hover:opacity-90 transition-opacity">
        New Booking
      </motion.button>
    </motion.div>
  )
