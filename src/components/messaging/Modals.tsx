...
export function NewConversationModal({ onClose, onConversationReady }: NewConversationModalProps) {
  ...
  const filtered = users.filter(u => {
    ...
    return `${u.first_name ?? ''} ${u.last_name ?? ''}`.toLowerCase().includes(q) || u.role.toLowerCase().includes(q)
  })

  const handleSelect = async (u: MessagableUser) => {
    if (starting) return
    setStarting(u.user_id)
    try {
      const conv = await messagingService.createOrGetConversation({ target_user_id: u.user_id, booking_id: u.booking_id })
    ...
    <ModalShell onClose={onClose}>
      ...
      <div className="px-4 pt-4 pb-3 flex items-center justify-between shrink-0 border-b border-white/[0.06]">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-spartan text-white text-sm tracking-[0.15em] uppercase">New Message</h3>
        </div>
        <button type="button" onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/5 text-white/40 hover:text-white transition-colors"><X size={14} /></button>
      </div>
      ...
      <div className="px-3 py-3 shrink-0">
        <div className="relative">
          ...
          <input ref={searchRef} type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or role…"
      ...
      {/* List */}
      <div className="flex-1 overflow-y-auto px-2 pb-3 min-h-0 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full">
        {loading && <div className="flex items-center justify-center py-12"><Loader2 size={18} className="animate-spin text-[var(--color-cyan)]/40" /></div>}
        {!loading && error   && <p className="ff-body text-white/30 text-xs text-center py-12">{error}</p>}
        {!loading && !error && filtered.length === 0 && <p className="ff-body text-white/20 text-xs text-center py-12">{search ? 'No contacts match' : 'No contacts available'}</p>}
        {!loading && !error && filtered.map(u => {
          const initials    = `${u.first_name?.[0] ?? '?'}${u.last_name?.[0] ?? '?'}`.toUpperCase()
          const isStarting  = starting === u.user_id
          return (
            <motion.button key={u.user_id} whileHover={{ backgroundColor: 'rgba(255,255,255,0.04)' }} whileTap={{ scale: 0.98 }}
              onClick={() => handleSelect(u)} disabled={!!starting}
              className="w-full flex items-center gap-3 px-3 py-3 rounded-xl text-left transition-colors disabled:opacity-60">
...
export function NewGroupModal({ onClose, onGroupCreated }: NewGroupModalProps) {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true)
    try {
      const result = await messagingService.createGroup({ name: groupName.trim(), member_ids: [...selected] })
      onGroupCreated(result.group_id)
    } catch { setError('Failed to create group'); setCreating(false) }
  }
  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <div className="px-3 pt-3 pb-2 shrink-0">
        <input type="text" value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="Group name…" maxLength={100}
        ...
        <div className="px-3 pb-2 flex flex-wrap gap-1.5 shrink-0">
          {selectedUsers.map(u => (
            <button key={u.user_id} type="button" onClick={() => toggle(u.user_id)}
              ...
              {u.first_name} {u.last_name}<X size={10} />
            </button>
          ))}
        </div>
      )}
      ...
      <div className="px-3 pb-2 shrink-0">
        <div className="relative">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/25 pointer-events-none" />
          <input ref={searchRef} type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search people…"
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 min-h-0 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full">
        {loading ? (
          <div className="flex items-center justify-center py-12"><Loader2 size={18} className="animate-spin text-[var(--color-cyan)]/40" /></div>
        ) : filtered.length === 0 ? (
          ...
          filtered.map(u => {
            ...
            const initials = `${u.first_name?.[0] ?? '?'}${u.last_name?.[0] ?? '?'}`.toUpperCase()
            return (
              <motion.button key={u.user_id} whileTap={{ scale: 0.98 }} onClick={() => toggle(u.user_id)}
      ...
      <div className="px-3 py-3 border-t border-white/[0.06] shrink-0">
        <button type="button" onClick={handleCreate} disabled={!groupName.trim() || selected.size === 0 || creating}
