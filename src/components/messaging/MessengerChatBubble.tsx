...
import { X, Minus, ChevronDown, Send, Check, CheckCheck, Loader2 } from 'lucide-react'
...
export default function MessengerChatBubble({ conversationId, index }: MessengerChatBubbleProps) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      // silently fail in bubble; full error handling in MessagingShell
    } finally {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev =>
        prev.map(m => (m.id === optimisticId ? toMessage(saved) : m))
      )
    } catch {
