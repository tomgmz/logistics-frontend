...
export default function MessengerFloatingPanel() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const convs  = rawConvs.map(toConversation)
  ...
  useEffect(() => { if (isPanelOpen) fetchAll() }, [isPanelOpen, fetchAll])
  ...
  const unified: PanelItem[] = [
    ...conversations.map(c => ({ kind: 'dm'    as const, data: c, lastAt: c.last_message?.created_at ?? '' })),
    ...groups.map(g       => ({ kind: 'group'  as const, data: g, lastAt: g.last_message?.created_at ?? g.created_at })),
  ].sort((a, b) => b.lastAt.localeCompare(a.lastAt))

  const filtered = unified.filter(item => {
    const q    = search.toLowerCase()
    const name = item.kind === 'dm'
      ? `${item.data.participants[0].first_name} ${item.data.participants[0].last_name}`.toLowerCase()
      : item.data.name.toLowerCase()
    return name.includes(q) && (tab === 'All' || item.data.unread_count > 0)
  })
        ...
        >
          ...
          <div className="px-4 pt-4 pb-2 flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => { closePanel(); router.push('/messages') }} title="Open full Messages" className="p-1.5 rounded-lg hover:bg-white/5 text-white/40 hover:text-white transition-colors">
                <Expand size={14} />
              </button>
              <button type="button" onClick={closePanel} className="p-1.5 rounded-lg hover:bg-white/5 text-white/40 hover:text-white transition-colors">
                <X size={14} />
              </button>
            </div>
          </div>
          ...
          <div className="px-3 pb-2 shrink-0">
            <div className="relative">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search"
                className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl pl-9 pr-3 py-2 ff-body text-xs text-white placeholder:text-white/20 focus:outline-none focus:border-[var(--color-cyan)]/30 transition-colors" />
            </div>
          </div>
          ...
          <div className="px-3 pb-2 flex items-center gap-1.5 shrink-0">
            {(['All', 'Unread'] as Tab[]).map(t => (
              <button key={t} type="button" onClick={() => setTab(t)}
                className={`ff-body text-xs px-3 py-1 rounded-full transition-colors ${tab === t ? 'bg-[var(--color-cyan)]/12 text-[var(--color-cyan)] border border-[var(--color-cyan)]/20' : 'text-white/40 hover:bg-white/5 hover:text-white/60'}`}>
                {t}
              </button>
            ))}
          </div>
          ...
          {/* List */}
          <div className="flex-1 overflow-y-auto px-2 pb-2 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
            {loading ? (
              <div className="flex items-center justify-center py-10"><Loader2 size={16} className="animate-spin text-[var(--color-cyan)]/40" /></div>
            ) : filtered.length === 0 ? (
              <p className="ff-body text-white/20 text-xs text-center py-10">{search ? 'No results found' : 'No conversations yet'}</p>
            ) : (
              filtered.map(item =>
                item.kind === 'dm' ? (
                  ...
                  <PanelGroupItem key={`group-${item.data.id}`} group={item.data}
                    onSelect={() => { openGroupChat(item.data.id, item.data.unread_count); closePanel() }} />
                )
              )
            )}
          </div>
          ...
          <div className="px-4 py-2.5 border-t border-white/[0.05] shrink-0">
            <button type="button" onClick={() => { closePanel(); router.push('/messages') }}
              className="ff-body text-[11px] text-[var(--color-cyan)]/60 hover:text-[var(--color-cyan)] transition-colors text-center w-full">
              See all
            </button>
          </div>
        </motion.div>
...
function PanelDmItem({ conv, currentUserId, onSelect }: { conv: Conversation; currentUserId: string; onSelect: () => void }) {
  ...
  return (
    <motion.button whileHover={{ backgroundColor: 'rgba(255,255,255,0.04)' }} whileTap={{ scale: 0.98 }}
      onClick={onSelect} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <span className={`ff-body text-[13px] truncate ${hasUnread ? 'text-white' : 'text-white/65'}`}>{p.first_name} {p.last_name}</span>
          {last && <span className="ff-body text-[10px] text-white/22 shrink-0">{formatConversationTime(last.created_at)}</span>}
        </div>
        <div className="flex items-center gap-1.5">
          <p className={`ff-body text-[11px] truncate flex-1 ${hasUnread ? 'text-white/55' : 'text-white/22'}`}>
            {last?.sender_id === currentUserId && <span className="text-[var(--color-cyan)]/50">You: </span>}
            {last?.body ?? ''}
          </p>
          {hasUnread && (
            <span className="shrink-0 w-4 h-4 rounded-full bg-[var(--color-cyan)] flex items-center justify-center">
              <span className="ff-body text-[9px] font-bold text-[var(--color-bg)]">{conv.unread_count > 9 ? '9+' : conv.unread_count}</span>
            </span>
          )}
        </div>
      </div>
...
function PanelGroupItem({ group, onSelect }: { group: Group; onSelect: () => void }) {
  ...
  return (
    <motion.button whileHover={{ backgroundColor: 'rgba(255,255,255,0.04)' }} whileTap={{ scale: 0.98 }}
      ...
      <div className="relative shrink-0">
        ...
        {isPending && <span className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-amber-400 border-2 border-[var(--color-bg)]" />}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <span className={`ff-body text-[13px] truncate ${hasUnread ? 'text-white' : 'text-white/65'}`}>{group.name}</span>
          {last && <span className="ff-body text-[10px] text-white/22 shrink-0">{formatConversationTime(last.created_at)}</span>}
        </div>
        <div className="flex items-center gap-1.5">
          <p className={`ff-body text-[11px] truncate flex-1 ${hasUnread ? 'text-white/55' : 'text-white/22'}`}>
            {isPending ? <span className="text-amber-400/70">Invite pending</span>
              : last?.body ?? <span className="text-white/20 italic">{group.members.filter(m => m.status === 'accepted').length} members</span>}
          </p>
          {hasUnread && (
            <span className="shrink-0 w-4 h-4 rounded-full bg-[var(--color-cyan)] flex items-center justify-center">
              <span className="ff-body text-[9px] font-bold text-[var(--color-bg)]">{group.unread_count > 9 ? '9+' : group.unread_count}</span>
            </span>
          )}
        </div>
      </div>
