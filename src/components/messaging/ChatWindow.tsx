...
export default function ChatWindow({ conversation, currentUserId, onBack, onMessageSent }: ChatWindowProps) {
  ...
  const { broadcastTyping } = useMessagingRealtime({
    ...
    onReadReceipt: ({ conversation_id, read_at }) => {
      ...
      setMessages(prev =>
        prev.map(m => m.sender_id === currentUserId && !m.read_at ? { ...m, read_at } : m)
      )
    },
          ...
          <AnimatePresence mode="wait">
            {isOtherTyping ? (
              <motion.p
                ...
                className="ff-body text-[11px] text-[var(--color-cyan)] flex items-center gap-1"
              >
                <TypingDots />
                typing…
              </motion.p>
              ...
              <motion.p
                ...
                exit={{ opacity: 0, y: -4 }}
                transition={{ duration: 0.15 }}
                className="ff-body text-[11px] leading-tight"
              >
                {isParticipantOnline
        ...
        <AnimatePresence>
          {isOtherTyping && (
            <motion.div
              ...
              className="flex items-end gap-2 max-w-[72%] mr-auto"
            >
  ...
  return (
    <span className="flex items-center gap-[3px]">
      {[0, 1, 2].map(i => (
        <motion.span
          key={i}
          className="w-1.5 h-1.5 rounded-full bg-[var(--color-cyan)]/60 inline-block"
          animate={{ y: [0, -4, 0] }}
