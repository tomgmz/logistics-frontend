...
// Audit/system log codes are stored abbreviated (`fleet_admin_created`,
// `gm_auto_approved`, `admin_activity`); expand them for display only.
const CODE_TOKENS: [RegExp, string][] = [
  [/(^|_)fleet_admin(?=_|$)/g,      '$1fleet_manager'],
  [/(^|_)operations_admin(?=_|$)/g, '$1operations_manager'],
  [/(^|_)admin(?=_|$)/g,    '$1administrator'],
  [/(^|_)gm(?=_|$)/g,       '$1general_manager'],
  [/(^|_)ops(?=_|$)/g,      '$1operations_manager'],
]

export function expandCode(code: string | null | undefined): string {
  if (!code) return ''
  return CODE_TOKENS.reduce((s, [rx, rep]) => s.replace(rx, rep), code)
}
